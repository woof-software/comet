...
export default migration("1707394874_configurate_and_ens", {
    ...
    const actions = [
      {
        ...
        calldata: utils.defaultAbiCoder.encode(
          ["address", "uint256"],
          [roninl1NativeBridge.address, USDCAmountToBridge]
        ),
      },
      {
  ...
  async verify(
    ...
    const { comet, rewards, USDC } = await deploymentManager.getContracts();
    ...
    expect(await USDC.balanceOf(comet.address)).to.be.equal(exp(1, 6));
    // 6.
    const ENSResolver = await govDeploymentManager.existing(
      'ENSResolver',
      ENSResolverAddress
