...
export default migration('1779290648_update_non_usd_based_to_v2_factory', {
  ...
  async enact(deploymentManager: DeploymentManager) {
    ...
    const mainnetActions = [
      ...
      // 5. Update WBTC Comet factory to the new one
      {
        contract: configurator,
        signature: 'setFactory(address,address)',
        args: [WBTC_COMET, COMET_FACTORY_V2],
      },
      // 6. Set service patch version of the extension delegate for the WBTC Comet
      {
        contract: configurator,
        signature: 'setExtensionDelegate(address,address)',
      ...
      // 8. Update wstETH Comet factory to the new one
      {
        contract: configurator,
        signature: 'setFactory(address,address)',
        args: [WSTETH_COMET, COMET_FACTORY_V2],
      },
      // 9. Set service patch version of the extension delegate for the wstETH Comet
      {
        contract: configurator,
        signature: 'setExtensionDelegate(address,address)',
        args: [WSTETH_COMET, WSTETH_EXT],
      },
      // 10. Deploy and upgrade wstETH Comet to a new version of Comet
      {
        contract: cometAdmin,
        signature: 'deployAndUpgradeTo(address,address)',
...
Detailed information can be found on the corresponding [proposal pull request](https://github.com/Compound-Foundation/comet/pull/6).
...
The fifth proposal action updates the factory of the WBTC Comet to the new V2 factory.

The sixth proposal action sets the extension delegate for the WBTC Comet to the new service patch version.

The seventh proposal action deploys and upgrades the WBTC Comet to the new service patch version.

The eighth proposal action updates the factory of the wstETH Comet to the new V2 factory.

The ninth proposal action sets the extension delegate for the wstETH Comet to the new service patch version.

The tenth proposal action deploys and upgrades the wstETH Comet to the new service patch version.
`;
