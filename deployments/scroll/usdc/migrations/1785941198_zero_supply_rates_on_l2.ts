...
export default migration('1785941198_zero_supply_rates_on_l2', {
  ...
  async enact(deploymentManager: DeploymentManager, govDeploymentManager: DeploymentManager) {
    ...
    const mainnetActions = [
      // Gas limit bumped above the 1_000_000 used in past single-comet Scroll migrations:
      // this batch has 5 actions (4 rate setters + deployAndUpgradeTo) vs. their 2-3.
      {
        contract: scrollMessenger,
        signature: 'sendMessage(address,uint256,bytes,uint256)',
        args: [bridgeReceiver.address, 0, proposalData, 2_500_000],
        value: exp(0.2, 18),
      },
    ];
