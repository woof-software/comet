...
export default migration('1723551632_add_ezeth_as_collateral', {
  async prepare(deploymentManager: DeploymentManager) {
    const _ezETHPriceFeed = await deploymentManager.deploy(
      ...
      [
        EZETH_TO_ETH_PRICE_FEED_ADDRESS,  // ezETH / ETH price feed
        ETH_TO_USD_PRICE_FEED_ADDRESS,    // ETH / USD price feed
        8,                                // decimals
        'ezETH / USD price feed'          // description
      ]
    );

    return { ezETHPriceFeedAddress: _ezETHPriceFeed.address };
  },

  enact: async (deploymentManager: DeploymentManager, govDeploymentManager: DeploymentManager, { ezETHPriceFeedAddress }) => {
    ...
    newPriceFeed = ezETHPriceFeedAddress;
