...
export default migration('1739783281_configurate_and_ens', {
  ...
  enact: async (
    ...
    const _approveUSDTCalldata = await USDT_L1.populateTransaction.approve(sonicL1GatewayBridge.address, USDTAmountToSeed);
    const _depositUSDTCalldata = await sonicL1GatewayBridge.populateTransaction.deposit(uidUSDT, USDT_L1.address, USDTAmountToSeed);
    const depositUSDTCalldata = utils.defaultAbiCoder.encode(
      ['address[]', 'bytes[]', 'uint256[]'],
      [[USDT_L1.address, sonicL1GatewayBridge.address], [_approveUSDTCalldata.data, _depositUSDTCalldata.data], [0, 0]]
    );
    ...
    const depositIdUSDT = await l2SonicBridge.userOperationId(delegatorAddress, uidUSDT);
    ...
    const setUSDTClaimData = await delegatorContract.populateTransaction.setClaimData(
      depositIdUSDT,
      USDT_L1.address,
      USDTAmountToSeed,
      USDT_L2.address,
    ...
    const l2ProposalData1 = utils.defaultAbiCoder.encode(
    ...
    const l2ProposalData2 = utils.defaultAbiCoder.encode(
      ['address[]', 'uint256[]', 'string[]', 'bytes[]'],
      [
        [
          // rewards.address,
    ...
    const actions = [
      ...
      // },
      // 3. Get USDT reserves from cUSDT contract
      {
        target: cUSDTAddress,
        signature: '_reduceReserves(uint256)',
        calldata: _reduceReservesCalldata
      },
      // 4. Transfer USDT to Delegator
      {
        contract: USDT_L1,
        signature: 'transfer(address,uint256)',
        args: [delegatorAddress, USDTAmountToSeed]
      },
      // 5. Bridge USDT from Ethereum Delegator to Sonic Delegator
      {
        target: delegatorAddress,
        signature: 'call(address[],bytes[],uint256[])',
        calldata: depositUSDTCalldata
      },
      // 6. Set Comet configuration + deployAndUpgradeTo new Comet, set Reward Config on Sonic
      {
  ...
  async verify(
    ...
    // );
    const storageSlotUSDT = utils.keccak256(
      utils.defaultAbiCoder.encode(['uint256', 'uint8'], [depositIdUSDT, 7])
    );
    ...
    const dataFromStorageUSDT = await govDeploymentManager.hre.ethers.provider.getStorageAt(
      sonicL1GatewayBridge.address,
      storageSlotUSDT
    );
    expect(dataFromStorageUSDT).to.not.be.equal(constants.HashZero);
    ...
    // expect(await comet.getReserves()).to.be.equal(USDTAmountToSeed);
