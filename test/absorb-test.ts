...
import { event, expect, exp, factor, defaultAssets, makeProtocol, mulPrice, mulFactor, portfolio, totalsAndReserves, wait, bumpTotalsCollateral, setTotalsBasic, makeConfigurator, takeSnapshot, SnapshotRestorer, MAX_ASSETS, divPrice, presentValue, presentValueSupply, principalValue, ZERO_ADDRESS } from './helpers';
...
describe('absorb', function () {
  ...
  describe('absorbing multiple users', function () {
    ...
    it('AbsorbDebt events are emitted for each user', async () => {
      ...
      // user1 reuses alice, an account with prior protocol activity, so her principal can carry
      // 1 wei of rounding dust from earlier present-value conversions that fresh accounts (user2,
      // user3) don't have — tolerate that dust instead of requiring bit-exact event args.
      const receipt = await absorbTxMultiple.wait();
      const debtEvents = receipt.events?.filter(e => e.event === 'AbsorbDebt') ?? [];

      const expectedByBorrower: Record<string, [bigint, bigint]> = {
        [user1.address]: [basePaidOutUser1, valueOfBasePaidOutUser1],
        [user2.address]: [basePaidOutUser2, valueOfBasePaidOutUser2],
        [user3.address]: [basePaidOutUser3, valueOfBasePaidOutUser3],
      };

      for (const user of [user1, user2, user3]) {
        const event = debtEvents.find(e => e.args?.borrower === user.address);
        expect(event, `AbsorbDebt event missing for ${user.address}`).to.not.be.undefined;
        expect(event!.args!.absorber).to.equal(absorber.address);
        const [expectedBasePaidOut, expectedValueOfBasePaidOut] = expectedByBorrower[user.address];
        expect(event!.args!.basePaidOut).to.be.approximately(expectedBasePaidOut, 1);
        expect(event!.args!.usdValue).to.be.approximately(expectedValueOfBasePaidOut, 100);
      }
    });
