...
describe('Absorb Integration Tests', function () {
  ...
  it('should demonstrate automatic liquidation type selection', async () => {
    ...
    console.log('\n   Testing different liquidation scenarios:');
    
    // Scenario 1: Healthy account
    await (comet as any).setBasePrincipal(user.address, exp(1000, 6)); // Positive balance
    ...
    console.log(`   Healthy account - Liquidatable: ${isLiquidatable}, Partially: ${isPartiallyLiquidatable}`);

    // Scenario 2: Partially liquidatable account
    await (comet as any).setBasePrincipal(user.address, -exp(4000, 6)); // Moderate debt
    ...
    console.log(`   Partially liquidatable - Liquidatable: ${isLiquidatable}, Partially: ${isPartiallyLiquidatable}`);

    // Scenario 3: Bad debt account
    await (comet as any).setBasePrincipal(user.address, -exp(10000, 6)); // High debt
    await (comet as any).setCollateralBalance(user.address, assetInfo.asset, exp(1, 18)); // Low collateral
    await (comet as any).updateAssetsInExternal(user.address, assetInfo.asset, 0, exp(1, 18));
    
    isLiquidatable = await comet.isLiquidatable(user.address);
    isPartiallyLiquidatable = await (comet as any).isPartiallyLiquidatable(user.address);
    const isBadDebt = await (comet as any).isBadDebt(user.address);
    console.log(`   Bad debt - Liquidatable: ${isLiquidatable}, Partially: ${isPartiallyLiquidatable}, Bad Debt: ${isBadDebt}`);

    console.log('   ✅ All scenarios tested successfully');
  });

  // Epic 7 AC1: Test mathematical accuracy of formulas (13), (14), (10)
  ...
  // Epic 7 AC2: Test edge cases and boundaries
  ...
  // Epic 7 AC3: Test dust position handling
  it('should test dust position handling (Epic 1 AC3)', async () => {
    const { comet, users: [user] } = await makeProtocol({
      storefrontCoefficient: factor(0.8),
      assets: {
        USDC: { decimals: 6, initialPrice: 1 },
  ...
  // Epic 7 AC4: Test multiple partial liquidations
  ...
  // Epic 7 AC5: Test ecosystem compatibility
