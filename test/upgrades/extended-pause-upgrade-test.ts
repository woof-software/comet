...
describe('extended pause upgrade test', function () {
  ...
  before(async function () {
    ...
    // Upgrade ConfiguratorProxy to latest Configurator implementation
    ...
    const latestConfiguratorImpl = await LatestConfigurator.deploy();
    await latestConfiguratorImpl.deployed();

    const configuratorProxy = await ethers.getContractAt(
      'ConfiguratorProxy',
      CONFIGURATOR_ADDRESS
    );
    await configuratorProxy
      .connect(configuratorProxyAdmin)
      .upgradeTo(latestConfiguratorImpl.address);
