...
import { expectRevertCustom, usesAssetList } from './utils';
...
scenario('Comet#numAssets > market has at least one collateral asset', {}, async ({ comet }) => {
  expect(await comet.numAssets()).to.be.greaterThan(0);
});

scenario('Comet#governor > points to timelock', {}, async ({ comet, timelock }) => {
  expect(await comet.governor()).to.equal(timelock.address);
});

scenario('Comet#governor > is not the same address as pause guardian', {}, async ({ comet }) => {
...
scenario('Comet#initializeStorage > reverts if already initialized', {}, async ({ comet }) => {
  await expectRevertCustom(comet.initializeStorage(), 'AlreadyInitialized()');
});

scenario('Comet#configuration > matches configurator parameters', {}, async ({ comet, configurator }) => {
  const existingConfig = normalizeStructOutput(await configurator.getConfiguration(comet.address));
  ...
  const convertedExistingConfig = {
    ...restExistingConfig,
    supplyPerSecondInterestRateSlopeLow: supplyPerYearInterestRateSlopeLow / SECONDS_PER_YEAR,
    supplyPerSecondInterestRateSlopeHigh: supplyPerYearInterestRateSlopeHigh / SECONDS_PER_YEAR,
    supplyPerSecondInterestRateBase: supplyPerYearInterestRateBase / SECONDS_PER_YEAR,
    borrowPerSecondInterestRateSlopeLow: borrowPerYearInterestRateSlopeLow / SECONDS_PER_YEAR,
    borrowPerSecondInterestRateSlopeHigh: borrowPerYearInterestRateSlopeHigh / SECONDS_PER_YEAR,
    borrowPerSecondInterestRateBase: borrowPerYearInterestRateBase / SECONDS_PER_YEAR
  };
  ...
  const cometConfig = {
    ...
    supplyPerSecondInterestRateSlopeLow: (await comet.supplyPerSecondInterestRateSlopeLow()).toBigInt(),
    supplyPerSecondInterestRateSlopeHigh: (await comet.supplyPerSecondInterestRateSlopeHigh()).toBigInt(),
    supplyPerSecondInterestRateBase: (await comet.supplyPerSecondInterestRateBase()).toBigInt(),
    borrowKink: (await comet.borrowKink()).toBigInt(),
    borrowPerSecondInterestRateSlopeLow: (await comet.borrowPerSecondInterestRateSlopeLow()).toBigInt(),
    borrowPerSecondInterestRateSlopeHigh: (await comet.borrowPerSecondInterestRateSlopeHigh()).toBigInt(),
    borrowPerSecondInterestRateBase: (await comet.borrowPerSecondInterestRateBase()).toBigInt(),
    ...
    baseTrackingSupplySpeed: (await comet.baseTrackingSupplySpeed()).toBigInt(),
    baseTrackingBorrowSpeed: (await comet.baseTrackingBorrowSpeed()).toBigInt(),
...
scenario('Comet#getAssetInfoByAddress > reverts if asset does not exist', {}, async ({ comet }) => {
  const nonExistentAsset = ethers.Wallet.createRandom().address;

  await expect(comet.getAssetInfoByAddress(nonExistentAsset)).to.be.revertedWithCustomError(comet, 'BadAsset');
});

scenario(
  'Comet#assetsIn > correctly sets and clears _reserved bits for assets with offset >= 16',
  {
    filter: async (ctx: CometContext) => await usesAssetList(ctx)
  },
  async ({ comet, configurator, proxyAdmin, actors }, context) => {
    ...
    if (currentNumAssets < REQUIRED_NUM_ASSETS) {
      ...
      const currentConfiguration = normalizeStructOutput(await configurator.getConfiguration(comet.address));
      ...
      for (let i = 0; i < assetsToAdd; i++) {
        ...
        if (i === assetsToAdd - 1) {
          ...
          supplyAmount =
            BigInt(getConfigForScenario(context).supplyCollateral) * 10n ** BigInt(newAssetConfig.decimals);
      ...
      supplyAmount = BigInt(getConfigForScenario(context).supplyCollateral) * assetInfo.scale.toBigInt();
      await context.sourceTokens(supplyAmount, targetAssetAddress, albert.address);
    }
    ...
    await albert.withdrawAsset({ asset: assetInfo.asset, amount: supplyAmount });
    ...
    expect(await comet.collateralBalanceOf(albert.address, assetInfo.asset)).to.equal(0);
  }
);
