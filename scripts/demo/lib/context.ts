...
export async function loadDemo() {
  ...
  const roots = JSON.parse(readFileSync(rootsPath, 'utf8')) as { comet: string; seizureView: string };
  const cometAddress = process.env.COMET_ADDRESS ?? roots.comet;
  ...
  const seizureView = LiquidationSeizureView__factory.connect(roots.seizureView, provider);
  ...
  return { hre, comet, module, adapter, seizureView, wethInfo, wethFeed, usdc, weth, deployer, lender, borrower };
}
