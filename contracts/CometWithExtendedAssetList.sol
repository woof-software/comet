...
contract CometWithExtendedAssetList is CometMainInterface {
     ...
     * @param account The address to calculate LHF for
    ...
    function getMinimalDebt(address account) override public view returns (uint256) {
        ...
        // Get current debt in base units and USD value
        ...
        // Bad debt - full liquidation
        if (debtValue > totalCollateralLF) {
            return debtBase;
        }

        // No collateral - full liquidation
        ...
        // If target exceeds current debt - no liquidation needed
        if (targetDebtValue >= debtValue) {
            return 0;
        }

        // Minimal debt in USD → in base units
