...
contract CometWithPartialLiquidation is CometMainInterface {
    ...
    /// @notice Coefficient for partial liquidation target health factor
    ...
    constructor(Configuration memory config) {
        ...
        // Set partial liquidation coefficient
    ...
    /**
     * @notice Get the liquidation health factor for an account
     ...
     */
    function getTargetHF(address account) public view returns (uint) {
     ...
     */
    function isBadDebt(address account) public view returns (bool) {
        int104 principal = userBasic[account].principal;
     ...
     */
    function getMinimalDebt(address account) public view returns (uint256) {
        int104 principal = userBasic[account].principal;
        ...
        for (uint8 i = 0; i < numAssets; i++) {
            if (!isInAsset(assetsIn, i, _reserved)) continue;
     ...
     */
    function isPartiallyLiquidatable(address account) public view returns (bool) {
        int104 principal = userBasic[account].principal;
        if (principal >= 0) return false; // No debt
     ...
     */
    function collateralForMinimalDebt(address account) public view returns (address[] memory assets, uint256[] memory amounts) {
        int104 principal = userBasic[account].principal;
        if (principal >= 0) {
    ...
    function absorbPartialInternal(address absorber, address account, uint256 debtToRepay) internal {
        ...
        totalRepaidValue = seizeCollateral(absorber, account, assetsIn, _reserved, debtValue);

        // Convert repaid value back to base units
        uint256 deltaBalance = divPrice(totalRepaidValue, basePrice, uint64(baseScale));
        int256 newBalance = oldBalance + signed256(deltaBalance);
        ...
        // Update assetsIn flags for remaining collateral
        for (uint8 i = 0; i < numAssets; ) {
            if (isInAsset(assetsIn, i, _reserved)) {
                AssetInfo memory assetInfo = getAssetInfo(i);
                address asset = assetInfo.asset;
                uint128 currentBalance = userCollateral[account][asset].balance;
                
                // Update assetsIn based on current balance
                if (currentBalance == 0) {
                    if (i < 16) {
                        userBasic[account].assetsIn &= ~(uint16(1) << i);
                    } else if (i < 24) {
                        userBasic[account]._reserved &= ~(uint8(1) << (i - 16));
                    }
                }
            }
            unchecked { i++; }
    ...
    /**
     * @dev Seize collateral from user account to cover debt
     */
    function seizeCollateral(address absorber, address account, uint16 assetsIn, uint8 _reserved, uint256 debtValue) internal returns (uint256 totalRepaidValue) {
